...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for effectA in actionA.effects:
            for effectB in actionB.effects:
                effectA_str = str(effectA)
                effectB_str = str(effectB)
                if ("~" in effectA_str) and ("~" not in effectB_str):
                    if effectA_str == "~"+effectB_str:
                        return True
                elif ("~" in effectB_str) and ("~" not in effectA_str):
                    if effectB_str == "~"+effectA_str:
                        return True
        return False
    ...
    def _interference(self, actionA, actionB):
        ...
        # TODO: implement this function
        def cross_check(A,B):
            for effectA in A.effects:
                for preconditionB in self.parents[B]:
                    exprA_str = str(effectA)
                    exprB_str = str(preconditionB)
                    if ("~" in exprA_str) and ("~" not in exprB_str):
                        if exprA_str == "~"+exprB_str:
                            return True
                    elif ("~" in exprB_str) and ("~" not in exprA_str):
                        if exprB_str == "~"+exprA_str:
                            return True
            return False

        interferenceA = cross_check(actionA, actionB)
        interferenceB = cross_check(actionB, actionA)

        if interferenceA or interferenceB:
            return True
        else:
            return False
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _negation(self, literalA, literalB):
        ...
        # TODO: implement this function
        if (literalB.op == "~") and (literalA==literalB.args[0]):
            return True
        elif (literalA.op == "~") and (literalB==literalA.args[0]):
            return True
        else:
            return False
class PlanningGraph:
    ...
    def level_cost(self, goal):
        ...
        for literal_layer in self.literal_layers:
            if goal in list(literal_layer.children.keys()):
                return i
            i+=1
    ...
    def h_setlevel(self):
        ...
        i=0
        for literal_layer in self.literal_layers:
            # print(list(literal_layer.children.keys()))
            dic=literal_layer.parent_layer.children
            current_literals = [list(x)[0] for x in (dic.values())]

            allGoalsMet = True
            for goal in self.goal:
                if goal not in current_literals:
                    allGoalsMet = False
            if not allGoalsMet:
                i+=1
                continue
